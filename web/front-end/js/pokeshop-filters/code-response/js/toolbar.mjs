import { getPokemons } from './pokemon-storage.js';
...
function loadSortSelection(sortSelected) {
  ...
  if (sortSelected) document.querySelector(sortSelected).classList.add('selected');
  ...
  highestPriceBtn.onclick = () =>
    sortEvent('.toolbar-list-highest-price', 'Menor Preço', 'highestPrice');

  const lowestPriceBtn = dropdown.querySelector('.toolbar-list-lowest-price');
  lowestPriceBtn.onclick = () =>
    sortEvent('.toolbar-list-lowest-price', 'Maior Preço', 'lowestPrice');
}
...
function hideDropdown() {
  ...
  items.map(i => document.querySelector(i)).forEach(i => i.classList.remove('selected'));
...
function loadTypeFilterSelection() {
  ...
  const types = getPokemons(sortingMethod, [], 'all')
    ...
    .sort()
    .map(
      t => `<li><span class="background-${t} ${isSelected(t)}" data-filter="${t}">${t}</span></li>`,
    )
    .join('');
  ...
  const offset =
    typeFilterElement.offsetLeft + typeFilterElement.offsetWidth - toolbarListElement.offsetWidth;
